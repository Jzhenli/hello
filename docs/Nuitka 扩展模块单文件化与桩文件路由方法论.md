...
PKG_NAME = "your_package" # 替换为你的包名
RELEASE_DIR = f"release_{PKG_NAME}"
# 1. 执行 Nuitka 编译 (生成单一 .pyd/.so)
os.system(f'python -m nuitka --module {PKG_NAME} --include-package={PKG_NAME} --output-dir=dist')
# 2. 自动探测编译产物名称 (适配不同 Python 版本和平台后缀)
mod_files = glob.glob(f"dist/{PKG_NAME}.*.*") 
mod_file = os.path.basename(mod_files[0]) if mod_files else None
...
with open(f"{RELEASE_DIR}/__init__.py", "w", encoding="utf-8") as f:
    f.write(INIT_PY_TEMPLATE.format(pkg_name=PKG_NAME, mod_file=mod_file))
with open(f"{RELEASE_DIR}/__main__.py", "w", encoding="utf-8") as f:
    f.write(MAIN_PY_TEMPLATE.format(pkg_name=PKG_NAME))
