...
class UDPProtocolCodec:
    ...
    def encode(self, payload: bytes, sequence: Optional[int] = None) -> bytes:
        if len(payload) > MAX_UDP_PACKET_SIZE - self.HEADER_SIZE - 2:
            raise ValueError(
                f"Payload too large: {len(payload)} bytes, "
                f"maximum allowed: {MAX_UDP_PACKET_SIZE - self.HEADER_SIZE - 2} bytes"
            )
        ...
        logger.debug(
            f"Encoded packet: seq={sequence}, payload_len={len(payload)}, crc=0x{crc:04X}"
        )
        return packet

    def decode(self, data: bytes) -> Tuple[int, bytes]:
        min_length = self.HEADER_SIZE + 2
        if len(data) < min_length:
            raise ValueError(
                f"Packet too short: {len(data)} bytes, minimum {min_length} bytes"
            )
        ...
        if magic != self.MAGIC_NUMBER:
            raise ValueError(
                f"Invalid magic number: 0x{magic:04X}, expected 0x{self.MAGIC_NUMBER:04X}"
            )

        expected_length = self.HEADER_SIZE + payload_length + 2
        if len(data) < expected_length:
            raise ValueError(
                f"Length mismatch: packet {len(data)} bytes, expected {expected_length} bytes"
            )

        payload = data[self.HEADER_SIZE : self.HEADER_SIZE + payload_length]

        received_crc = struct.unpack(
            ">H", data[self.HEADER_SIZE + payload_length :]
        )[0]
        calculated_crc = crc16_x25(data[: self.HEADER_SIZE + payload_length])

        if received_crc != calculated_crc:
            raise ValueError(
                f"CRC check failed: received 0x{received_crc:04X}, "
