...
├── .github/
│   └── workflows/
│       └── build-linux.yml           ← CI/CD 工作流（支持 armv7/aarch64 多架构）
├── apps/
│   ├── data-collector/               ← 应用1
...
工作流文件位于 `.github/workflows/build-linux.yml`，支持以下特性：

- **多架构支持**：armv7 (ARM32) 和 aarch64 (ARM64)
- **智能按需构建**：根据 Git 标签自动判断构建类型
- **手动触发**：支持通过 `workflow_dispatch` 选择架构、构建类型等参数

### 构建类型矩阵

| 触发方式 | BUILD_TYPE | 产物包含 Python | 产物包含应用 |
|---------|------------|:--------------:|:----------:|
| `shared-python/v*` 标签 | `python-only` | ✅ | ❌ |
| `<app>/v*` 标签 | `app-only` | ❌ | ✅ |
| 无效标签（apps/下无对应目录） | `skip` | ❌ | ❌ |
| 手动触发 Workflow | `full` | ✅ | ✅ |

### 架构配置

| 架构 | PBS Target | PyPI 源 |
|------|-----------|---------|
| armv7 | `armv7-unknown-linux-gnueabihf` | piwheels.org |
| aarch64 | `aarch64-unknown-linux-gnu` | 标准 PyPI |

详细配置请参考实际工作流文件：`.github/workflows/build-linux.yml`
